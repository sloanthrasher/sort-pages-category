		...
		<table id="the-list" class="widefat fixed">
			...
			<tbody>
				...
				foreach ($pages as $page) {
					error_log("spp_display_admin_page page: " . print_r($page, true));

					$page_categories = wp_get_post_terms($page->ID, 'category', array('fields' => 'ids'));
					error_log("spp_display_admin_page page_categories: " . print_r($page_categories, true));
					...
					error_log("spp_display_admin_page opts: " . print_r($opts, true));

					foreach ($opts as $opt) {
						error_log("spp_display_admin_page opt: " . print_r($opt, true) . " -- category found: " . in_array($opt['id'], $page_categories) ? ' selected' : ' not selected');
...
//	error_log("assign_category_to_page: page_id = $page_id, category_ids = " . print_r($category_ids, true));
	if (!is_array($category_ids) || empty($category_ids)) {
//		error_log("assign_category_to_page: category_ids is not an array, or it is empty");
		return;
	}
//	error_log("assign_category_to_page: \$category_ids[$page_id] Is Array? " . (is_array($category_ids[$page_id]) ? 'Yes' : 'No'));
//	$cat_ids = (is_array($category_ids[$page_id]) ? $category_ids[$page_id] : array($page_id => $category_ids));
	$cat_ids = $category_ids[$page_id];
//	error_log("assign_category_to_page: page_id = $page_id, cat_ids = " . print_r($cat_ids, true));
	$result = wp_set_post_terms($page_id, $cat_ids, 'category');
//	error_log("assign_category_to_page: result = " . print_r($result, true) . ",\ncat_ids = " . print_r($cat_ids, true));
	...
	$page_categories = wp_get_post_terms($page_id, 'category', array('fields' => 'ids'));
//	error_log( "assign_category_to_page: " . in_array($page_id, $page_categories) ? 'in array' : 'not in array');
...
//		error_log("spp_saveOrderAndCategory - Page ID: " . $pageId . ", Index: " . $index);
